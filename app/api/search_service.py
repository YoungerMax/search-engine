...
def _search_context(q: str, limit: int, offset: int) -> dict[str, Any] | None:
...
def _rank_web_rows(rows: list[tuple[Any, ...]], *, context: dict[str, Any]) -> list[dict[str, Any]]:
...
def _rank_news_rows(rows: list[tuple[Any, ...]], *, context: dict[str, Any]) -> list[dict[str, Any]]:
            ...
            for row in rows:
                ...
                if context["total_terms"]:
                    ...
                    if matched_terms == context["total_terms"]:
                        score += 40.0
                fallback_results.append({"title": "", "description": "", "url": "", "score": score})
            fallback_results.sort(key=lambda item: item["score"], reverse=True)
            page = fallback_results[offset : offset + limit]
            return {"results": page, "count": max(len(fallback_results), offset + len(page))}

    ranked_results = _rank_web_rows(rows, context=context)
...
def perform_news_search(*, q: str, limit: int = 20, offset: int = 0) -> dict[str, Any]:
    ...
    return {"results": page, "count": max(len(ranked), offset + len(page))}
